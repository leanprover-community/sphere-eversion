...
theorem IsOpen.nhdsSet_eq_principal {s : Set α} (h : IsOpen s) : 𝓝ˢ s = 𝓟 s :=
  by
...
theorem IsClosed.nhdsSet_le_sup {t : Set α} (h : IsClosed t) (s : Set α) :
  ...
  calc
    ...
    _ = 𝓝ˢ (s ∩ t) ⊔ 𝓟 (tᶜ) := by rw [(is_open_compl_iff.mpr h).nhdsSet_eq_principal]

theorem IsClosed.nhdsSet_le_sup' {t : Set α} (h : IsClosed t) (s : Set α) :
    𝓝ˢ s ≤ 𝓝ˢ (t ∩ s) ⊔ 𝓟 (tᶜ) := by rw [Set.inter_comm]; exact h.nhds_set_le_sup s

theorem eventually_nhdsSet_iff {p : α → Prop} : (∀ᶠ x in 𝓝ˢ s, p x) ↔ ∀ x ∈ s, ∀ᶠ y in 𝓝 x, p y :=
  by rw [nhdsSet, eventually_Sup, Set.ball_image_iff]
...
theorem Filter.eventually_nhdsSet_prod_iff {α β : Type _} [TopologicalSpace α] [TopologicalSpace β]
    ...
    (∀ᶠ q in 𝓝ˢ (s ×ˢ t), p q) ↔
      ∀ x ∈ s,
        ∀ y ∈ t,
          ∃ (pa : α → Prop) (ha : ∀ᶠ x' in 𝓝 x, pa x') (pb : β → Prop) (hb : ∀ᶠ y' in 𝓝 y, pb y'),
...
theorem eventually_nhdsSet_Union₂ {α ι : Type _} [TopologicalSpace α] {p : ι → Prop} {s : ι → Set α}
...
theorem eventually_nhdsSet_iUnion {α ι : Type _} [TopologicalSpace α] {s : ι → Set α}
  ...
  simpa using @eventually_nhdsSet_Union₂ _ _ _ (fun i => True) s P
