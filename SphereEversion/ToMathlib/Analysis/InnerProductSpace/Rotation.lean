...
def rot (p : ℝ × E) : E →L[ℝ] E :=
  (ℝ ∙ p.2).subtypeL ∘L (orthogonalProjection (ℝ ∙ p.2) : E →L[ℝ] ℝ ∙ p.2) +
    ...
    Real.sin (p.1 * Real.pi) • (ω.crossProduct p.2).toContinuousLinearMap
...
theorem rot_zero (v : E) : ω.rot (0, v) = ContinuousLinearMap.id ℝ E :=
  by
  ext w
  simpa [rot] using (orthogonalProjection_add_orthogonalProjection_orthogonal (ℝ ∙ v) w).symm
...
theorem inner_rot_apply_self (p : ℝ × E) (w : E) (hw : w ∈ (ℝ ∙ p.2)ᗮ) : ⟪ω.rot p w, p.2⟫ = 0 :=
  ...
  have H₂ : (orthogonalProjection (ℝ ∙ p.2)ᗮ w : E) = w :=
    congr_arg (coe : (ℝ ∙ p.2)ᗮ → E) (orthogonalProjection_mem_subspace_eq_self ⟨w, hw⟩)
  have H₃ : ⟪w, p.2⟫ = 0 := by
...
theorem isometry_on_rot (t : ℝ) (v : Metric.sphere (0 : E) 1) (w : (ℝ ∙ (v : E))ᗮ) :
  ...
  simp [rot, orthogonalProjection_mem_subspace_orthogonalComplement_eq_zero w.prop, this]

theorem isometry_rot (t : ℝ) (v : Metric.sphere (0 : E) 1) : Isometry (ω.rot (t, v)) :=
  by
...
theorem injOn_rot_of_ne (t : ℝ) {x : E} (hx : x ≠ 0) : Set.InjOn (ω.rot (t, x)) (ℝ ∙ x)ᗮ :=
  ...
  have : 0 < cos (t * π) ^ 2 + sin (t * π) ^ 2 * ‖x‖ ^ 2 :=
    ...
    rw [cos_sq']
    rw [show 1 - sin (t * π) ^ 2 + sin (t * π) ^ 2 * ‖x‖ ^ 2 = 1 + sin (t * π) ^ 2 * (‖x‖ ^ 2 - 1)
        by ring]
    have I₁ : ‖x‖ ^ 2 - 1 > -1 := by linarith
    have I₂ : sin (t * π) ^ 2 ≤ 1 := sin_sq_le_one (t * π)
    have I₃ : 0 ≤ sin (t * π) ^ 2 := sq_nonneg _
