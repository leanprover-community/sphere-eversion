...
theorem range_affine_equiv_ball {p c : F} {s r : ℝ} (hr : 0 < r) :
    (range fun x : ball p s => c +ᵥ homothety p r (x : F)) = ball (c + p) (r * s) :=
  by
  ext
...
@[simp]
theorem norm_coe_ball_lt (r : ℝ) (x : ball (0 : F) r) : ‖(x : F)‖ < r := by cases' x with x hx;
  simpa using hx

theorem mapsTo_homothety_ball (c : F) {r : ℝ} (hr : 0 < r) :
    MapsTo (fun y => homothety c r⁻¹ y -ᵥ c) (ball c r) (ball 0 1) :=
  by
  intro y hy
  replace hy : r⁻¹ * ‖y - c‖ < 1
