...
initialize
  registerTraceClass.1 `filter_inst

initialize
  registerTraceClass.1 `filter_inst_type
...
unsafe def tactic_view_goal {γ} (local_c : tc local_collection γ) (target_c : tc expr γ) :
  ...
  tc.stateless fun ft => do
    let fi ← tactic.get_bool_option `trace.filter_inst false
    let fft := if fi then filter_type.no_instances else ft
    let fi' ← tactic.get_bool_option `trace.filter_inst_type false
    ...
    let lcs ← List.filterM (filter_local fft) lcs
    ...
    pure <|
        h "ul" [key g, className "list pl0 font-code"] <|
          case_tag ++ [h "li" [key u_n] [h "span" [cn "goal-vdash b"] ["⊢ "], t_comp]] ++ lchs
...
end WidgetOverrideSe

attribute [implemented_by widget_override_se.term_goal_widget] widget.term_goal_widget

attribute [implemented_by widget_override_se.tactic_state_widget] widget.tactic_state_widget
