...
lemma _root_.continuous.path_trans'' {X : Type*} [uniform_space X] [separated_space X]
  ...
  continuous ↿(λ (p : X × I) s, trans' (γ p.1 p.2) (γ' p.1 p.2) p.2 s) :=
begin
  have hγ0 : ∀ x, tendsto_uniformly (λ p : X × I, γ p.1 p.2) (λ _, f x) (𝓝 (x, 0)),
  { intro x, convert continuous.tendsto_uniformly (λ p : X × I, γ p.1 p.2) hγ _,
    ext x, rw [hγ0] },
  have hγ'1 : ∀ x, tendsto_uniformly (λ p : X × I, γ' p.1 p.2) (λ _, f x) (𝓝 (x, 1)),
  { intro x, convert continuous.tendsto_uniformly (λ p : X × I, γ' p.1 p.2) hγ' _,
    ext x, rw [hγ'1] },
  refine continuous.if_le _ _ continuous_snd continuous_fst.snd _,
  { rw [continuous_iff_continuous_at],
    rintro ⟨⟨x, t⟩, s⟩,
    refine continuous_at.comp_div_cases (λ (p : X × I) s, (γ p.1 p.2).extend s)
      continuous_at_fst (continuous_at_subtype_coe.comp continuous_at_snd)
      (continuous_at_subtype_coe.comp continuous_at_fst.snd) _ _,
    { intro h,
      refine continuous_at.extend (λ p : (X × I), γ p.1 p.2)
        hγ.continuous_at continuous_at_fst continuous_at_snd },
    { intro h,
      obtain rfl : t = 0 := subtype.ext h,
  ...
  { rw [continuous_iff_continuous_at],
    rintro ⟨⟨x, t⟩, s⟩,
    apply continuous_at.comp_div_cases (λ (p : X × I) s, (γ' p.1 p.2).extend s)
      ...
      (continuous_at_const.sub $ continuous_at_subtype_coe.comp continuous_at_fst.snd) _ _,
    { intro h,
      refine continuous_at.extend (λ p : (X × I), γ' p.1 p.2)
        hγ'.continuous_at continuous_at_fst continuous_at_snd },
    { intro h,
      obtain rfl : t = 1 := subtype.ext (sub_eq_zero.mp h).symm,
      apply filter.tendsto.extend,
      dsimp only, rw [(proj_Icc_surjective _).map_top, extend_zero],
      refine (tendsto_prod_top_iff $ λ p : (X × I), γ' p.1 p.2).mpr (hγ'1 x) } },
