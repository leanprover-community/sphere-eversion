...
begin
  ...
  let φ : ℝ → E → ℝ := sorry,
  let ψ : ℝ → E → ℝ := sorry,
  have H₁ : ∀ᶠ t near Iic (0 : ℝ), ∀ x, P t x ∧ φ t x = t,
  {
    sorry },
  have H₂ : ∀ᶠ t near Ici (1 : ℝ), ∀ x, ¬ P t x ∧ ψ t x = ψ 1 x,
  {
    sorry },
  have H₂' : ∀ x, ¬ P 1 x,
  sorry { exact λ x, (H₂.on_set 1 left_mem_Ici x).1 },
  have H₄ : ∀ t, ∀ x ∉ L.K₁, P t x ∧ φ t x = t,
  {
    sorry },
  have H₅ : ∀ᶠ x near L.K₀, ψ 1 x = 1,
  {
    sorry },
  ...
  { apply (h_t_0.and H₁).mono,
    rintros t ⟨ht, ht'⟩,
    apply jet_sec.ext',
    intros x,
    rw [h𝓕'_apply t x, if_pos (ht' x).1, (ht' x).2, ht], },
  { apply H₂.mono,
    intros t ht,
    apply jet_sec.ext',
    intros x,
    rw [h𝓕'_apply t x, if_neg (ht x).1, h𝓕'_apply 1 x, if_neg (H₂' x), (ht x).2] },
  ...
  { rw [nhds_set_union, eventually_sup] at h𝓖CA h_CA,
    ...
    { apply (h𝓖CA.2.eventually_nhds_set.and h_CA.2).mono,
      rintro x₀ hx₀,
      apply hx₀.2.congr,
      apply hx₀.1.mono,
      intros y hy,
      simp only [h𝓕'_apply, if_neg (H₂' _), hy],
      refl },
    { apply filter.eventually.union,
      ...
      { apply (H₅.eventually_nhds_set.and h𝓖K₀).mono,
        ...
        apply hx.mono,
        intros y hy,
        simp only [h𝓕'_apply, if_neg (H₂' y), hy] } } },
end
