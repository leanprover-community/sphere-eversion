...
lemma interval_oc_of_gt (h : a > b) : interval_oc a b = Ioc b a :=
...
begin
  ...
  [{ rw interval_oc_of_le hab at *,
   ...
   { rw interval_oc_of_gt hab at *,
     simp_rw interval_integral.integral_of_ge (le_of_lt hab),
     refine continuous_at.neg _ }];
  apply tendsto_integral_filter_of_dominated_convergence bound gc hF_meas (mem_of_nhds hF_meas) h_bound,
