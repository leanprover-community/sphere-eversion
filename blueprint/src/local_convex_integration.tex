...
γ_x$ is $Df(x)v$ for all $x$ is some set $U$. Obviously such loops can exist
...
on $U$ using the following construction.
...
\begin{proposition}[Theillière 2018]
  ...
  pair on $E$. Let $γ \co E × 𝕊^1 → F$ be a compactly
  supported $\mathcal{C}^1$ family of loops.
  Assume that $\overline{γ_x} = Df(x)v$ for all $x$ in some set $U$.

  For any positive $ε$, the function $f'$ obtained by corrugation of $f$ in
  direction $(π, v)$ using $γ$ with large enough oscillation number $N$
  satisfies:
  \begin{enumerate}
    \item\label{CP:C0}
      $∀ x, ‖f'(x) - f(x)‖ ≤ ε$
    \item\label{CP:kerpi}
      $∀ x, ‖(Df'(x) - Df(x))_{|\ker π}‖ ≤ ε$.
    \item\label{CP:v}
      $∀ x ∈ U, ‖Df'(x)v - γ(x, Nπ(x))‖ ≤ ε$
  \end{enumerate}
  In addition, all the differences estimated above vanish if $x$ is outside the
  support of $γ$ (in addition to being in $U$ in the last estimate).
\end{proposition}
...
\begin{lemma}
  ...
  \leanok
  \lean{rel_loc.formal_sol.improve_step_rel_t_eq_0, rel_loc.formal_sol.improve_step_rel_C,%
    rel_loc.formal_sol.improve_step_rel_compl_K₁, rel_loc.formal_sol.improve_step_c0_close,%
    rel_loc.formal_sol.improve_step_hol}
...
\begin{lemma}
  ...
  \leanok
  \lean{rel_loc.formal_sol.improve_rel_t_eq_0, rel_loc.formal_sol.improve_rel_C,%
    rel_loc.formal_sol.improve_rel_compl_K₁, rel_loc.formal_sol.improve_c0_close,%
    rel_loc.formal_sol.improve_hol}
