...
Note that in this whole text, every manifold is paracompact by definition.

\begin{lemma}
  \label{lem:nice_atlas}
  Let $M$ be manifold and $(V_j)_{j ∈ J}$ be a covering of $M$ by open sets.
  ...
  \begin{itemize}
    ...
    \item $⋃_i φ_i(B) = M$ where $B$ is the open unit ball in $E$.
  \end{itemize}
\end{lemma}
...
\begin{definition}
  ...
  \end{itemize}
  Such a tuple will be denote by $(φ, ψ)$ for brevity.
\end{definition}
...
\begin{proof}
  ...
  $B_F$ cover $N$. We then apply this lemma again to the cover given by all
  $f⁻¹(ψ_j(B_F))$.
\end{proof}

The general idea will be to apply the results of the previous chapters to
all the $ψ_{j(i)}⁻¹ ∘ f ∘ φ_i : E → F$ for some maps $f$. However we must be careful
that doing this for some $i$ does not ruin the setup for the next $i$.
This is easier to control using a distance function on the target manifold as in
...
\begin{lemma}
  ...
  $(φ, ψ)$ be some localisation data for $f$. Let $d$ be a distance function
  defining the topology of $N$. There exists some continuous positive
  function $ε : M → ℝ_{>0}$ such that:
  \[
