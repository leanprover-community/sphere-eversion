...
\begin{lemma}
  ...
  compact sets in $X$ and $Y$ respectively. Let $f : M → N$ be a smooth map such
  ...
  \begin{itemize}
    ...
    \item
      ...
      number $η$ such that, if $∀ x, ‖g(x) - ψ⁻¹∘f∘φ(x)‖ < η$ then
      $∀ m ∈ M, d(f(m), f'(m)) < ε(m)$.
  \end{itemize}
\end{lemma}

\begin{proof}
  In order to prove smoothness of $f'$, note that $M = φ(X) ∪ φ(L)^c$. Both
