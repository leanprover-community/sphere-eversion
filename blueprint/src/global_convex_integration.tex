...
\begin{definition}
  ...
  \lean{RelMfld.slice}
  \leanok
  Let $\Rel ⊂ J^1(M, N)$ be a first order partial differential relation.
  For any $σ = (x, y, φ)$ in $\Rel$ and any dual pair
  $(λ, v) ∈ T^*_xX × T_xX$, the slice of $\Rel$ at $(σ, λ, v)$ is.
  \[
      \Rel_{σ, λ, v} =
       \Conn_{φ(v)}\left\{w ∈ T_yY \;;\;
         \big(x,\; y,\; φ + (w - φ(v))⊗λ\big) ∈ \Rel\right\}
  \]
  where $\Conn_a A$ is the connected component of $A$ containing $a$. 
\end{definition}

In order to decipher this definition, it suffices to notice that 
$φ + (w - φ(v))⊗λ$ is the unique linear map from $T_xX$ to $T_yY$ which
coincides with $φ$ on $\ker λ$ and sends $v$ to $w$. In particular, $w = φ(v)$
gives back $φ$.
...
\subsection{Localisation of maps}%
\label{sub:LocalisationMaps}
...
All this allows us to modify a given map using one chart. But we need more
control in order to do it for all charts of some atlas.
...
\begin{proof}
  ...
  \[
    E \simeq B_E(y_j, r) \simeq B_{n,j} \subseteq M. \qedhere
  \]
\end{proof}
...
\begin{proof}
  \uses{lem:exists_cont_diff_of_convex, lem:exists_chart, lem:exists_atlas}
  \Cref{lem:exists_atlas} provides an atlas $ψ_i : ℝ^n → M'$ for $i$ in some set
  ...
  of $x₀$.
  \Cref{lem:exists_chart} gives a smooth open embedding 
...
\subsection{Localisation of jet spaces and relation}%
\label{sub:LocalisationMaps}
