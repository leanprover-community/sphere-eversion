...
\begin{lemma}
  ...
  Let $M$ be manifold and $(V_j)_{j ∈ J}$ be a cover of $M$ by open sets.
  There exists a normed vector space $E$, a finite or countable set $ι$ and
  a family of smooth open embeddings $φ : ι × E → M$ such that
  \begin{itemize}
    \item For each $i$ there is some $j$ such that $φ_i(E) ⊂ V_j$
    \item $i ↦ φ_i(E)$ is a locally finite collection of sets in $M$.
    \item $⋃_i φ_i(B_E) = M$ where $B_E$ is the open unit ball in $E$.
  \end{itemize}
\end{lemma}
...
\begin{proof}[Proof of Theorem~\ref{thm:open_ample}]
  ...
  \begin{itemize}
    ...
    \item
      $F_{n+1, 1}$ is holonomic on $K_{≤ n} := \bigcup_{i ≤ π(n)} φ_i(\bar B_E)$~;
    \item
  ...
  Otherwise we have $π(n+1) > π(n)$. For brevity, we set $i = π(n+1)$
  and $C = A ∩ φ_{π(n)}(\bar B_E) ∩ K_{≤ n}$.

  For any $t$, since $d(\bs F, \bs F_{n, t}) < ε$ by induction hypothesis,
  $\bs F_{n, t}$ sends $φ_i(E)$ into $ψ_{j(i)}(F)$.

  \Cref{def:transfer_map} then
  turns $F_n$ into a homotopy of section $G$ of $J¹(E, F)$.
  The new homotopy $F_{n+1}$ will be constructed by updating $F_n$ using some
  homotopy $\bar H$ of sections of $J¹(E, F)$. In order to ensure
  $d(\bs F, \bs F_{n+1}) < ε$, it suffices to ensure
  $d(\bs F_n, \bs F_{n+1}) < ε - d(\bs F, \bs F_n)$.
  This will hold as soon as, for all $e$ and $t$, $‖\bs G_t(e) - \bs \bar
  H_t(e)‖ < η$ for some positive $η$ given by
  \Cref{lem:updating}.
  According to \Cref{lem:transfer}, each $G_t$ is a formal solution of
  the relation $\Rel_i$ induced by $\Rel$ in $J¹(E, F)$ via $φ_i$ and
  $ψ_{j(i)}$, and $G_1$ is holonomic near $φ_i⁻¹(C)$.

  Hence \Cref{lem:h_principle_open_ample_loc} can be applied to $G₁$ with
  $K₀ = \overline{B_E}$, $K₁ = 2\overline{B_E}$ (the ball with radius $2$ around
  the origin in $E$) and $C = φ_i⁻¹(A ∪ K_{≤ n-1})$, with precision $η$. This
  way we get a homotopy $t ↦ H_t$ starting at $H₀ = G₁$, relatve to $C$ and the
  complement of $K₁$ and such that $H₁$ is holonomic on $\overline B_E$.
  Let $ρ \co E → [0, 1]$ be a smooth cut-off function which equals one on a
  neighborhood of $\overline B_E$ and vanishes outside $2B_E$.

  We define a new homotopy $\bar H$ of sections of $J¹(E, F)$ as:
  \[
    \bar H_t(x) = \begin{cases}
      G_{2t/(2 - ρ(x))}(x) & \text{if $t ≤ 1 - ρ(x)/2$}\\
      H_{2t/(2 - ρ(x)) - 1}(x) & \text{if $t ≥ 1 - ρ(x)/2$}
    \end{cases}
  \]
  Near $K₀$, where $ρ$ equals one, this $\bar H$ is simply the concatenation of $G$ and
  $H$. Outside $K₁$ where $ρ$ vanishes, it is simply $G$.
  We now transfer back this homotopy $\bar H$ to get $F_{n+1}$. \Cref{lem:transfer}
  ensures that we get a homotopy of formal solutions that is holonomic on
  $K_{≤ n}$.
