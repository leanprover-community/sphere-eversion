...
\leanok
\lean{analysis.convex.caratheodory, https://github.com/leanprover-community/mathlib/blob/caratheodory2/src/analysis/convex/caratheodory.lean#L159}
  If a point $x$ of $ℝ^d$ lies in the convex hull of a set $P$,
  then $x$ can be written as the convex combination of at most $d + 1$
  affinely independant points in $P$ with positive coefficients.
\end{lemma}
...
\begin{lemma}
  \label{lem:int_cvx}
  If a point $x$ of $ℝ^d$ lies in the convex hull of an open set $P$,
  then $x$ can be written as the convex combination of exactly $d + 1$
  affinely independant points in $P$ with only positive coefficients.
\end{lemma}

\begin{proof}
  \uses{lem:caratheodory}
  \Cref{lem:caratheodory} gives affinely independant points
