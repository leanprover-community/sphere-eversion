...
\begin{proof}
  ...
  The map $q ↦ e^*_i(q)$ is continuous (actually smooth) on $Π_i B_δ(p_i)$
...
\begin{proof}
  \uses{lem:loop_of_hull, lem:cont_convex_hull}
  ...
  neighborhood of $x_0$.
  \Cref{lem:cont_convex_hull} guarantees that this loop surrounds $g(x)$
...
\begin{proof}
  \uses{lem:caratheodory, lem:cont_convex_hull}
  ...
  \Cref{lem:cont_convex_hull}.

  If $x'$ is in a sufficiently small neighborhood of $x$,
  \Cref{lem:cont_convex_hull} gives smooth weight functions $w_j$
