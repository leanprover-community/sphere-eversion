...
In all this document, $E$ and $F$ will denote finite-dimensional
real vector spaces.
All this chapter is devoted to proving the following proposition.
...
needs to be a discontinuous piecewise constant map. The assumption that $v$ is
...
consistently.
The key feature of the above definition is that the $t$ parameter,
in addition to allowing to cut out the corrugation
process in the next chapter, brings a satisfied or refund guarantee,
...
\begin{proof}[Proof of Proposition~\ref{prop:loops}]
  ...
  still surrounding and taking value in $Ω$.
