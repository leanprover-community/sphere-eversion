...
The following lemma provides a convenient alternate characterisation of
being surrounded.
...
\begin{proof}
  ...
  \begin{itemize}
    \item the convex hull of an affine basis is the intersection of the
    preimages of $[0, \infty)$ over the corresponding $d+1$ barycentric
    coordinate maps,
    \item barycentric coordinates are continuous open maps,
    \item for a continuous open map, the preimage of the interior is the
    interior of the preimage,
    \item the interior of a finite intersection is the intersection of the
    interiors,
    \item the interior of the interval $[0, \infty)$ is $(0, \infty)$.
  \end{itemize}
\end{proof}
...
\begin{proof}
  ...
  Carathéodory's lemma \Cref{lem:caratheodory} gives affinely independent
  ...
  $\epsilon > 0$ such that
  \[
    h_\epsilon (\hat b) \subseteq P.
  \]
  Since $c$ belongs to the interior
  of the convex hull of $\hat b$ and since $\epsilon > 0$, we have:
  \[
    \Conv(\hat b) \subseteq \IntConv (h_\epsilon (\hat b))
  \]
  where $\Conv$ and $\IntConv$ denote the convex hull and its interior
  respectively.

  The required basis is thus $b = h_\epsilon (\hat b)$.
\end{proof}
